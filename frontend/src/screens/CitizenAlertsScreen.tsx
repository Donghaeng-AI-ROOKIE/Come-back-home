...
export default function CitizenAlertsScreen() {
  ...
  const live = data?.[0];
  const activeCaseId = live?.caseId ?? DEMO_CASE_ID;
    ...
    <SafeAreaView style={styles.safe} edges={['top']}>
      ...
      <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
        ...
        <AlertCard
          tone="red"
          badge="수색 중(0.8km 이내)"
          title={live ? `[서울시 ${live.area}] 78세 김oo 어르신` : '[서울시 서대문구] 78세 김oo 어르신'}
          body="현재 내 위치 반경 진입 확률이 매우 높습니다.\n터치하여 예상 구역을 확인해 주세요."
          onPress={() => navigation.navigate('AlertDetail', { caseId: activeCaseId })}
        />
        <AlertCard
          tone="green"
          badge="상황 종료"
          title="[서울시 마포구] 7세 이oo 아동"

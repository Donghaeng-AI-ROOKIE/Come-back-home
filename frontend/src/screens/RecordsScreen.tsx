...
export default function RecordsScreen() {
  ...
  return <SafeAreaView style={styles.safe} edges={['top']}><StatusBar style="dark" /><FigmaStatusBar /><ScrollView contentContainerStyle={styles.content} refreshControl={<RefreshControl refreshing={isRefetching} onRefresh={refetch} />}>
    ...
    <View style={styles.hero}>{isLoading ? <Text style={styles.muted}>불러오는 중…</Text> : <Text style={styles.heroText}>{data?.level_label ?? '안심 동행'} · Lv.{data?.level ?? 1}</Text>}</View>
    ...
    <Text style={styles.section}>최근 산책 기록</Text>
...
const styles = StyleSheet.create({
  safe: { flex: 1, backgroundColor: '#FFFFFF' }, content: { paddingHorizontal: 23, paddingTop: 30, paddingBottom: 40 }, title: { fontFamily: type.familyBold, fontSize: 20, color: '#000000' }, hero: { height: 103, borderRadius: 10, backgroundColor: color.figmaField, alignItems: 'center', justifyContent: 'center', marginTop: 19 }, heroText: { fontFamily: type.familySemiBold, fontSize: 16, color: '#525253' }, muted: { fontFamily: type.family, fontSize: 12, color: color.figmaGray },
  ...
  section: { fontFamily: type.familySemiBold, fontSize: 18, color: '#000000', marginTop: 40 }, badges: { height: 103, borderRadius: 10, backgroundColor: color.figmaField, marginTop: 14, flexDirection: 'row', alignItems: 'center', justifyContent: 'space-around' }, badge: { alignItems: 'center', width: 70 }, badgeIcon: { fontSize: 28 }, badgeLabel: { fontFamily: type.family, fontSize: 10, color: '#525253', marginTop: 3, textAlign: 'center' },

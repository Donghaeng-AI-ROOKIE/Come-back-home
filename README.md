...
- 출력: 이름·나이·유형·집 좌표·끌림점(근거 태그 포함)·카테고리 선호(`preferred_targets`)·행동 노트·축별 근거(`axis_evidence`)·축 점수(`axis_scores`)를 가진 `Persona`
...
- `attraction`: 옛집, 직장, 선호 대상 장소처럼 좌표화할 끌림점. 추출 시 장소 유형(`place_type`)과 근거(`evidence`, enum 3종: 과거 실종 때 실제 발견 `previous_missing_found` / 보호자 관찰 `caregiver_report` / 언급만 `mention_only`)를 함께 분류합니다 — 근거는 발화 이해가 필요해 추출 단계에서만 가능하고 이후 복원이 불가하기 때문입니다. "자동문", "지하철"처럼 특정 장소가 아닌 카테고리 선호는 지오코딩하지 않고 `Persona.preferred_targets`로 분리 저장합니다(이원화).
...
- 같은 백그라운드 트리거에서 `phase0/route_familiarity_compiler.py`가 자전적 기억 목적지(`autobiographical_destination_pull`) 유래 끌림점의 route_familiarity(경로별 익숙함)도 함께 채점합니다. 닫힌 후보 목록(끌림점 라벨) 각각에 축 점수와 동일한 A~E 5단계(0.1~0.9, 팀 확정 기준표)를 매기는 "list 출력" 패턴이며, 근거 발화가 없거나 실패하면 빈 결과로 폴백해 Phase 2의 거리 기반 근사가 대신합니다. 혼자 자주 가는 곳(`routine_destinations`) 유래 끌림점은 이미 기본 익숙함이 적용돼 컴파일 대상에서 제외됩니다.
...
예측 직전에 `Persona.preferred_targets`(지하철·편의점 같은 좌표화 불가 카테고리 선호)를 LKP 주변에서 실제 POI로 매칭해 이번 예측 스코프의 끌림점으로 합류시킵니다(`geo/poi.py`). 회의록 원칙("모든 지하철역 일괄 가중치 금지, 등록된 대상만 반영")에 따라 반경 3km·대상당 근접 3개 상한을 둡니다. 데이터원은 카카오 로컬 카테고리 검색(코드가 없는 대상은 키워드 검색 폴백)이며, 키가 없거나 API가 실패하면 매칭 없이 기존 끌림점만으로 예측을 계속합니다. 매칭 결과는 저장된 Persona를 바꾸지 않습니다 — LKP가 바뀌는 재실행마다 새로 매칭됩니다.

### 2-1. EXAONE prior 생성

EXAONE은 좌표를 예측하지 않고 다음 구조화 파라미터만 반환합니다. 프롬프트의 끌림점 목록에는 근거 태그(과거 실종 때 실제 발견된 곳 / 보호자 관찰 / 언급만)가 함께 실려, "실제 발견지"와 "지나가듯 언급한 곳"이 같은 등급을 받지 않게 합니다.
...
3. 시민 제보에서 위치·시각·구체성(행동 단서) 을 바탕으로 신뢰도 `p`를 계산합니다. 시민 제보 사진 대조는 하지 않기로 확정되어 신뢰도 계산에서 제외됩니다(등록 사진 인상착의 추출은 Phase 1에서 유지).

...
챗봇이 채워야 하는 정보 항목 하나를 슬롯이라고 부릅니다. 현재 카탈로그는 22개이며 다음 세 종류의 저장 위치(`sink`)를 가집니다.

- `field`: 이름, 나이, 유형, 현재 집 등 Persona 기본 필드
- `attraction`: 옛집, 직장, 단골 장소처럼 좌표화할 끌림점
- `behavior`: 길을 잃었을 때의 행동, 보행 능력, 대인 반응 등 행동 노트

각 슬롯은 `label`, 대표 질문, `probes`, `filled_when`, SAR 근거인 `why`, `keywords`, 선택적 `risk`를 정의합니다. 검색용 `embed_text`는 `label + question + probes + keywords + why`로 구성되며, `filled_when`은 Mi:dm의 추출·충족 판정에 사용됩니다.
...
| 설계 항목 | 현재 판정 | 근거와 차이 |
|---|---|---|
| 적응형 온보딩으로 안정적 Persona 생성 | 대체로 일치 | 22개 슬롯, 히스토리-어웨어 검색, Mi:dm 추출·문장화, 확인 게이트, 지오코딩 구현 |

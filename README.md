...
치매 노인·지적장애인의 실종 시 이동 가능 구역을 확률적으로 예측하고, 해당 구역 안의 시민에게만 실종 알림을 보내 제보를 유도하는 수색 지원 서비스입니다.
...
챗봇이 채워야 하는 정보 항목 하나를 슬롯이라고 부릅니다. 카탈로그는 페르소나 축 고도화 회의록(몸축·마음축·행동축)을 따라 16개로 구성됩니다: 공통 8개(기본 필드 2 + 몸축 2 + 마음축 3 + 행동축 1) + 치매 특화 4개 + 발달장애 특화 4개. '자폐'는 발달장애 세트로 라우팅됩니다. 저장 위치(`sink`)는 세 종류입니다.

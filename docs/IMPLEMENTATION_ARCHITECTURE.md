...
실제 `exaone-mind-dem3` 호출은 예측당 최대 5회이다. 실 EXAONE 실험에서 상한 10회는 약 15초, 5회는 약 10초였고 해당 비교의 POA 차이는 시드 변동 범위였다. 이후 발동은 앞선 결과 풀에서 독립 표집한다. 풀도 비어 있으면 혼란도 `+0.2` 휴리스틱을 사용한다.

v2 계약은 `behavior`, `goal_label`, `confusion_level`, `status`를 반환한다. `sanitize_mind()`와 시뮬레이션은 검증된 목표와 혼란/상태로 `target_node`와 `kappa`를 갱신한다. `behavior`는 PR #109(2026-08-04 머지)로 보행에 연결됐다 — 귀소 시도는 과거 거주지·직장 방위, 계속 배회는 매 스텝 무작위 방위, 은신·멈춤은 이동 종료이며 매핑 근거는 DEM-31·33·34다. `settings.mind_behavior_enabled`로 게이팅하며 **기본은 켜짐**이다(2026-08-04 전환). 켜기 전 기준선과 켠 뒤 실측은 `experiments/alert_cells/`에 있다 — 알림 셀 수 4h 기준 15.8→13.5개, 평균 이탈거리는 0.57→0.55km로 사실상 불변이다. 즉 후보 지역의 물리적 크기가 아니라 그 안의 우선순위가 바뀌어 뒤질 구역이 좁아진다. 다만 "더 잘 찾는가"는 이 지표로 판정할 수 없다.

혼란도는 여전히 **LLM 이 낸 상·중·하를 고정 수치(0.85/0.6/0.35)로 변환해 쓴다** (`guardrail.CONFUSION_LEVELS`). 규칙 산정(보호자 진술의 기능 수준·정보량으로 계산)으로 옮기는 작업은 `feat/mind-training-data-quality` 브랜치의 미커밋 상태로만 존재하며 **develop 에 머지되지 않았다** — 대외 서술에서 "규칙으로 산정한다"고 쓰면 안 된다.

이 PR 이 바꾼 것은 혼란도의 **소비처**다. 종전에는 κ(갈림길 방향 집중도) 하나뿐이었고, 그 경로는 개별 궤적만 흔들 뿐 500명을 모은 POA 를 움직이지 못한다는 것이 실측으로 확인됐다. 그래서 `_recognizes_destination()` 을 추가해 **혼란한 워커는 목표 지점에 도착해도 알아보지 못하고 지나칠 수 있게** 했다(근거 CLM-0008·CLM-0015·CLM-0023). 발동 조건은 마음 재해석을 실제로 거친 워커로 한정한다 — 초기 기본값 0.5 는 "아직 판정 없음"이지 "중간 정도 혼란"이 아니므로, 이를 함께 소비하면 관측 효과의 3분의 2 가 플레이스홀더에서 나온다(2026-08-04 실측). 세기는 `settings.confusion_miss_strength` 로 조절한다.

**이 경로의 수치는 대부분 잠정값이다**: 구성개념(공간적 방향상실)은 문헌 근거가 있으나 κ·wobble·등급 매핑 값에는 출처가 없다. 전수 목록과 대외 인용 시 지켜야 할 구분은 [혼란도 수치 근거 정리](혼란도_수치_근거_정리.md) 참조.

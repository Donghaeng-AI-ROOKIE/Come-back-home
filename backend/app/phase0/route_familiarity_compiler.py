...
닫힌 후보 목록(끌림점 라벨) 각각에 상/중/하 등급을 매기는 형태로 설계한다
(goal_label 의 "실존 라벨만 인정" 가드레일 패턴을 리스트로 확장한 것).

axis_scoring 과 달리 다수결(runs 회 반복)은 하지 않는다 — 등급 3단계뿐이라
비결정성의 영향이 축 채점(5단계)보다 작고, 실패 시 그냥 빈 리스트로 폴백해
unfamiliarity() 의 거리 기반 근사가 대신하므로 재시도 비용을 들일 이유가 약하다.
"""
...
대해 대상자가 그 장소까지 가는 길을 얼마나 잘 아는지 상/중/하로 평가한다.

출력 규칙:
- JSON 객체 하나만 출력한다. JSON 밖에 어떤 문장도 쓰지 않는다.
- 키는 주어진 장소 후보 라벨 중에서만 고른다. 후보에 없는 장소를 지어내지 않는다.
- 발화에 언급이 없는 장소는 아예 키를 넣지 마라(추측 금지).
- 값은 "상"(자주 다녀서 잘 앎)/"중"(가끔 다녀봄)/"하"(잘 모름) 중 하나."""

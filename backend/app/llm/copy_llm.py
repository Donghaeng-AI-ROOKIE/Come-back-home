...
#: 🚨 "빠뜨리지 마라"가 맨 앞에 있는 것이 이 프롬프트의 핵심이다 (2026-08-06 실측).
#:
#: 처음에는 이 규칙이 아래 목록 중간에 한 줄로 있었다. 그때 통과율이 **55%**였고,
#: 특히 회피 지시("공원처럼 트인 곳은 피하셨을")는 **온도 0.0 에서도 0/5** 로 매번
#: 사라졌다 — 샘플링 흔들림이 아니라 지시가 닿지 않은 것이다. 온도를 0.4→0.0 으로
#: 낮춰도 75% 에서 멈췄고, 이 문단을 앞으로 끌어올리자 **100%** 가 됐다(5회×4케이스).
#: 온도로 고칠 수 있는 문제가 아니었다.
_SYSTEM = """\
...
가장 중요한 규칙: 원문에 나온 **장소 이름과 '피하셨을' 같은 회피 표현을 하나도 \
빠뜨리지 않는다.** 말투만 바꾸고 내용은 그대로 옮긴다.

그 밖에 지켜야 할 것:
- 원문에 없는 장소·시간·상태를 덧붙이지 않는다. 정보를 늘리는 게 아니라 \
말투만 다듬는 것이다.

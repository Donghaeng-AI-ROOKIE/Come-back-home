...
class Settings(BaseSettings):
    ...
    #        수치 — 가 그대로 유효).
    #   켬 = "귀소 시도" 집 방향 보행 / "은신·멈춤" 제자리 체류 / "계속 배회" 목표
    #        해제 후 무작위 방위. "끌림점 접근"은 goal_label 경로가 이미 처리한다.
    ...
    # 은신·멈춤 발동 시 제자리에 머무는 스텝 수. 체류 동안 시간이 흐르므로 게이지는
    # 계속 쌓이고(피로·혼란), 이동만 멈춘다. 거리 예산(total_km)은 건드리지 않는다 —
    # 체류로 스텝을 소진하는 것과 예산을 깎는 것을 함께 걸면 같은 억제를 두 번 건다.
    mind_hide_dwell_steps: int = 20
    # 체류 1스텝이 소비하는 시뮬 시간(분). 이동 스텝의 dt(엣지길이/속도)와 달리
    # 거리가 없으므로 고정값을 쓴다. 20스텝 × 3분 = 1시간 체류.
    mind_hide_dwell_dt_min: float = 3.0

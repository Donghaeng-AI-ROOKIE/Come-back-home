"""KT 믿음(Mi:dm) 엔드포인트 프로브 — OpenAI 호환 여부/형식 확인.

앱 임포트·외부 패키지 없이 stdlib 만으로 한 번 호출해 본다.
크리덴셜은 '환경변수'로 넘긴다(채팅/코드에 비밀키를 넣지 않기 위해).

실행:
  MIDM_BASE_URL="<endpoint URL>" \
  MIDM_MODEL="<endpoint ID>" \
  MIDM_API_KEY="<API key>" \
  python3 scripts/probe_midm.py
...
BASE = os.environ.get("MIDM_BASE_URL", "").rstrip("/")
MODEL = os.environ.get("MIDM_MODEL", "")
KEY = os.environ.get("MIDM_API_KEY", "")

if not (BASE and MODEL and KEY):
    raise SystemExit("환경변수 MIDM_BASE_URL / MIDM_MODEL / MIDM_API_KEY 를 모두 설정하세요.")

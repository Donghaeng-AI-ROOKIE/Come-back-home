...
# 라벨 풀 = 일반 명사 8 + 고유명사 8 (v8, 2026-08-07).
#
# 고유명사를 넣는 이유(정본 = mind_strata/results_label_swap.md): v5·dem3 실측에서
# goal 선택 정책이 일반 명사로만 학습돼 고유명사 라벨(실사용자 등록 형태)에는
# 일반화되지 않았다 — 임의 고유명사 goal 선택 0~4/15, 반면 "정릉" 접두만 14~15/15
# (베이스 사전지식 아티팩트). 고유명사가 target 뿐 아니라 routine·distractor 역할로도
# 등장해야 "고유명사 → 무조건 선택"이 아니라 **근거 게이팅**이 고유명사에서 학습된다
# (버킷 혼합이 자동으로 이를 보장 — scenario() 가 역할별로 같은 풀에서 뽑는다).
#
# 어휘 선정 제약 — HOLDOUT_LABEL_TERMS(아래) 및 GOLD_LABELS 와 겹치지 않는
# 서울 실존 동네·하천 이름만 사용.
PLACES = {
    "familiar": ["동네 경로당", "주간보호센터", "작업장", "동네 슈퍼", "마을회관", "단골 약국", "문화센터", "산책길 입구",
                 "길음 경로당", "수유 주간보호센터", "창동 복지관", "상계 마을회관", "불광 약국", "연신내 문화센터", "석관동 슈퍼", "봉천 경로당"],
    "goal": ["재래시장", "동네 성당", "버스 차고지", "작은 도서관", "체육관", "기차역 대합실", "공원 정자", "세탁소",
             "길음시장", "수유시장", "창동 성당", "상계 도서관", "불광시장", "회기시장", "사당 성당", "응암시장"],
    "water": ["하천 산책로", "공원 연못", "분수광장", "저수지 둘레길", "개울 다리", "수변공원", "낚시터 입구", "생태공원",
              "우이천 산책로", "중랑천 다리", "불광천 수변길", "도림천 둘레길", "안양천 보행교", "당현천 물가", "묵동천 산책로", "성내천 둘레길"],
    "quiet": ["건물 지하계단", "지하주차장 구석", "공원 수풀", "빈 상가 복도", "도서관 뒤편", "아파트 계단실", "창고 옆", "육교 아래",
              "길음역 지하보도", "수유 골목 창고", "상계 아파트 계단실", "연신내 빈 상가", "회기동 골목 안쪽", "사당 지하주차장", "봉천 육교 아래", "석관동 공터"],
    "visual": ["전광판 있는 광장", "조명가게 앞", "회전문 있는 건물", "대형 간판 앞", "분수 조명광장", "유리 엘리베이터", "네온사인 골목", "전철 승강장",
               "수유역 광장", "연신내 네온 골목", "창동역 전광판 앞", "사당역 승강장", "길음뉴타운 상가 간판", "불광역 회전문", "상계역 조명광장", "응암역 입구"],
    "route": ["매일 걷는 통학로", "센터 가는 골목길", "집 앞 산책길", "평소 출근길", "시장 가는 길", "강변 보행로", "마을버스 정류장", "복지관 오솔길",
              "길음역 가는 골목길", "수유시장 가는 길", "상계 둘레 산책로", "불광동 언덕길", "회기역 방면 골목", "사당동 오르막길", "봉천고개 보행로", "응암동 골목길"],
    "distractor": ["처음 보는 쇼핑몰", "낯선 공원", "옛 직장 터", "다른 동네 시장", "관광버스 정류장", "새로 생긴 광장", "낯선 지하철역", "먼 친척 동네",
                   "낯선 오류동 상가", "처음 보는 개봉동 공원", "먼 신정동 병원", "낯선 등촌동 시장", "모르는 암사동 골목", "가본 적 없는 둔촌동 상가", "낯선 목동 광장", "먼 화곡동 정류장"],
}

# 학습 코퍼스 금지 어휘 — main() 이 생성 후 전량 검사한다.
#   정릉·아리랑          데모 케이스(김순자·정릉동). 학습·평가 양쪽에서 격리 —
#                        데모가 학습에 들어가면 시연이 일반화 검증이 아니게 된다.
#   망원·성북·남대문      goal 고유명사 일반화 홀드아웃(probe_label_swap 평가 전용).
#   청량리·경동·면목      골드셋 어휘(GOLD_LABELS)의 지명 성분.
HOLDOUT_LABEL_TERMS = ("정릉", "아리랑", "망원", "성북", "남대문", "청량리", "경동", "면목")
...
def main(variants: int = 5, goal_variants: int = 16) -> None:
    ...
    # 홀드아웃 격리 검사 — 데모(정릉·아리랑)·goal 일반화 홀드아웃(망원·성북·남대문)·
    # 골드셋 지명이 어떤 행에도 새어들지 않았음을 전량 보장한다. 이게 깨지면
    # 시연·홀드아웃 평가가 일반화 검증이 아니게 되므로 생성 자체를 실패시킨다.
    for perspective, rows in outputs.items():
        for row in rows:
            text = json.dumps(row, ensure_ascii=False)
            for term in HOLDOUT_LABEL_TERMS:
                assert term not in text, \
                    f"홀드아웃 어휘 '{term}' 누출: {perspective} {row['id']}"

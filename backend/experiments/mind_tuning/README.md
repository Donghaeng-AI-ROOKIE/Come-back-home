...
목표는 치매 관련 원문 논문에서 검증된 행동 진술을 추출하고,
...
7. GitHub `mind_goldset`의 시나리오는 평가 전용이며 학습에 포함하지 않는다.
...
발달장애(DEV-*) 논문 21편의 코퍼스 페이지·클레임 30건과 그 파생 학습 샘플을 삭제했다.
- `claims/claims.jsonl` 63 → 33건, `corpus/manifest.jsonl` 34 → 13편
- `dataset/train_*.jsonl` 2594 → 1240행, `dataset/val_*.jsonl` 446 → 194행

**운영 중인 마음 모델(mind-v5) 어댑터는 삭제 전 혼합 코퍼스로 학습된 것이다.**
지금 남은 데이터셋으로는 그 어댑터가 재현되지 않는다 — 치매 단독 학습본이 필요하면
재학습해야 한다. 원본은 git 이력(추적 파일) 및 삭제 전 로컬 스냅샷에만 있다
(`corpus/pages/`·`dataset/`·`candidates.jsonl` 은 .gitignore 대상이라 git 복원 불가).
